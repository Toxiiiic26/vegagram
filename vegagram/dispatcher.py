...
from vegagram.raw.types import (
...
class Dispatcher:
    ...
    def __init__(self, client: "vegagram.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            ...
            return (
                await vegagram.types.Message._parse(
                    self.client,
                    update.message,
                    users,
                    chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await vegagram.types.CallbackQuery._parse(self.client, update, users, chats),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                vegagram.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                vegagram.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                vegagram.types.Poll._parse_update(self.client, update, users),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                vegagram.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                vegagram.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                vegagram.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ChatJoinRequestHandler
            )

        async def story_parser(update, users, chats):
            return (
                await vegagram.types.Story._parse(self.client, update.story, users, chats, update.peer),
                StoryHandler
            )

        async def pre_checkout_query_parser(update, users, chats):
            return (
                await vegagram.types.PreCheckoutQuery._parse(self.client, update, users),
                PreCheckoutQueryHandler
            )

        async def shipping_query_parser(update, users, chats):
            return (
                await vegagram.types.ShippingQuery._parse(self.client, update, users),
                ShippingQueryHandler
            )

        async def message_reaction_parser(update, users, chats):
            return (
                vegagram.types.MessageReactionUpdated._parse(self.client, update, users, chats),
                MessageReactionHandler
            )

        async def message_reaction_count_parser(update, users, chats):
            return (
                vegagram.types.MessageReactionCountUpdated._parse(self.client, update, users, chats),
                MessageReactionCountHandler
            )

        async def chat_boost_parser(update, users, chats):
            return (
                vegagram.types.ChatBoostUpdated._parse(self.client, update, users, chats),
                ChatBoostHandler
            )

        async def purchased_paid_media_parser(update, users, chats):
            return (
                vegagram.types.PurchasedPaidMedia._parse(self.client, update, users),
                ChatBoostHandler
            )
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except vegagram.StopPropagation:
                                raise
                            except vegagram.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.exception(e)
