...
import vegagram
from vegagram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~vegagram.Client.start`, :meth:`~vegagram.idle` and :meth:`~vegagram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that vegagram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`vegagram.compose`.
