...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~vegagram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~vegagram.Client.sign_in` or :meth:`~vegagram.Client.sign_up`).

        next_type (:obj:`~vegagram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~vegagram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~vegagram.Client.resend_code`.
    """
